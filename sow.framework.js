...
( function ( global, factory ) {
    ...
    if ( _IS_LOCAL_ !== "T" ) {
        ( function () {
            ...
            console.log = function () {
                if ( Sow.OS === "Mobile" ) return;
                if ( this.__log === false ) return;
    ...
    __export_module.onerror = function ( msg, url, line, col, error ) {
        console.elog( error, true );
        return true;
    };
    /** [/Catch Unhandled Javascript Errors]**/
    ...
    Sow.define( 'Sow', function () {
            ...
            hook: function ( schema, child ) {
                ...
                return {
                    ...
                    add: function ( name, fn ) {
                        ...
                        if ( schema ) {
                            ...
                            if ( typeof ( child ) === "undefined" ) {
                                _event[schema][name].fn.push( fn );
                                return this;
                            }
                            if ( !_event.dependancy[child] )
                                _event.dependancy[child] = {};
                            if ( !_event.dependancy[child][schema] )
                                _event.dependancy[child][schema] = [];
                            let index = _event.dependancy[child][schema].indexOf( name );
                            if ( index < 0 )
                                _event.dependancy[child][schema].push( name );
                            if ( !_event[schema][name][child] )
                                _event[schema][name][child] = [];
                            index = _event[schema][name][child].indexOf( name );
                            if ( index >= 0 )
                                _event[schema][name][child].splice( index, 1 );
                            _event[schema][name][child].push( fn );
                            return this;
                        }
                        if ( !_event[name] ) {
                            _event[name] = {
                    ...
                    firea: function () {
                        ...
                        let owner = _event[schema][name];
                        let ev = owner.fn, len = ev.length - 1;
                        ...
                        for ( let x in owner ) {
                            if ( x === "fn" ) continue;
                            if ( !owner[x] || typeof ( owner[x] ) !== "object" ) continue;
                            for ( let i = 0, l = owner[x].length; i < l; i++ ) {
                                let cpyfn = owner[x][i];
                                Sow.async( function () {
                                    cpyfn.apply( this, arg ); return;
                                }, 0 );
                            }
                        }
                        return this;
                ...
                remove: function ( schema, name ) {
                    ...
                    if ( typeof ( schema ) !== 'undefined' ) {
                        ...
                        if ( _event.dependancy[schema] ) {
                            for ( let p in _event.dependancy[schema] ) {
                                let parent = _event[p];
                                if ( !parent ) continue;
                                let farr = _event.dependancy[schema][p];
                                if ( !farr || farr === null || typeof ( farr ) !== "object" )
                                    continue;
                                for ( let x = 0, l = farr.length; x < l; x++ ) {
                                    delete parent[farr[x]][schema];
                                }
                            }
                            delete _event.dependancy[schema];
                        }
                        return this;
                    }
                    if ( _event[name] ) {
        ...
        return CREATE.aggregate( function () {
            ...
            return {
                ...
                remove: function ( name ) {
                    if ( _moduleGlobal.registered[name] )
                        delete _moduleGlobal.registered[name];
                    ...
                    if ( !args ) {
                        if ( this.browser.Promise.support ) {
                            private_worker.initiate( delay ).then( function () {
                                ...
                                console.elog( new Error( 'Error Message: ' + reason.message + '\r\nHandle rejected promise (' + reason.stack + ') here.' ) );
                            } );
                        } else {
                ...
                date: {
                    ...
                    format: ( new this.Assembler() ).Create.aggregate( function () {
                        ...
                        return function ( arg1, arg2 ) {
                            ...
                            return typeof df_worker.format[f] !== 'function' ? df_worker.format.default( date ) : df_worker.format[f]( date );
                        };
                    } ),
...
( function ( $ ) {
    ...
    $.fn.__val = $.fn.val;//keep original method for further use
    $.fn.val = function ( v ) {
        if ( this.length <= 0 ) {
    ...
    $.fn.__html = $.fn.html;//keep original method for further use
    $.fn.html = function () {
        if ( arguments.length <= 0 )
            return this.__html() || "";
        return this.__html.apply( this, Array.prototype.slice.call( arguments ) );
    };
    $.fn.htmla = function ( html ) {
        return this.css( {
            opacity: '0.0'
        } ).__html( html ).delay( 50 ).animate( {
            opacity: '1.0'
        }, 300 );
    };
...
console.log( atob( "JWNTYWZlIE9ubGluZSBXb3JsZCAoU09XKQ==" ), "font-weight: bold; font-size: 50px; color: rgb(64, 172, 43) !important; font-family: arial; text-shadow: -0.5px -0.5px 0 #000, 0.5px -0.5px 0 #000, -0.5px 0.5px 0 #000, 0.5px 0.5px 0 #000;" );
console.__log = false;
Sow.define( "Sow", function () {
    return {
        onXHRError: function () {
            ...
            return function ( jqXHR, uac, msg ) {
                try {
                    var status_code = jqXHR.getResponseHeader( "x-http-status-code" );
                    status_code = !status_code ? jqXHR.statusCode().status : parseInt( status_code );
                    if ( typeof ( uac ) === "number" ) {
                        if ( status_code === uac ) {
                            alert( msg || _status_map[status_code] );
                            Sow.hook( "Manager" ).firea( "onSginOut" );
                            return {
                                fw: false,
                                status_code: status_code,
                                status_description: _status_map[status_code]
                            };
                        }
                    }
                    return {
                        fw: true,
                        status_code: status_code,
                        status_description: _status_map[status_code] || msg
                    };
                } catch ( e ) {
                    return {
                        fw: true,
        ...
        loader: function () {
            var $elm = undefined, req = 0, isOpen = false;
            return {
                show: function ( ) {
                    if ( !$elm ) $elm = $( '.page-loader-wrapper' );
                    if ( req <= 0 ) {
                        req = 0; $elm.fadeIn( 'slow' );
                    }
                    req++;
                },
                hide: function () {
